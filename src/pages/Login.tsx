...
import { Sparkles, FileText, Brain, GraduationCap, TrendingUp, Sun, Moon } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row transition-colors duration-300 font-sans select-none ${isDark ? 'bg-[#050816] text-white' : 'bg-slate-50 text-slate-900'}`}>
      
      {/* Back to Home / Theme Toggle Header Row for Mobile */}
      <div className="absolute top-4 right-4 z-50 flex items-center gap-3">
        <button
          onClick={() => setIsDark(!isDark)}
          className={`p-2.5 rounded-full backdrop-blur-md transition-all duration-300 border ${
            isDark 
              ? 'bg-white/5 border-white/10 hover:bg-white/10 text-yellow-400' 
              : 'bg-black/5 border-black/10 hover:bg-black/10 text-violet-600'
          }`}
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-6 sm:p-12 relative overflow-hidden">
        ...
        {/* Glow ambient background sphere for right side */}
        ...
        <div className="w-full max-w-md relative z-10 flex flex-col items-center">
          ...
          <div className="flex md:hidden items-center gap-3 mb-8">
            ...
            <div className="text-left">
              <h1 className={`text-2xl font-black tracking-wider ${isDark ? 'text-white' : 'text-slate-900'}`}>
                LUMEN AI
              </h1>
