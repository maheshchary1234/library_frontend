...
export const Quiz: React.FC = () => {
  ...
  const handleSelectDocument = async (doc: DocumentInfo) => {
    ...
    setLoading(true);
    try {
      const data = await aiService.getQuiz(doc.id);
      setQuestions(data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="min-h-screen bg-slate-900 text-white p-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-teal-400">AI Quizzes</h2>
            <p className="text-slate-400 text-xs mt-1">Select a document to test your knowledge</p>
          </div>
          ...
          {selectedDoc ? (
            <div className="w-full space-y-8">
              <div className="text-center">
                <h3 className="text-lg text-slate-450">Active Quiz for:</h3>
              ...
              {loading ? (
                <div className="text-center py-12 space-y-4">
                  ...
                  <p className="text-slate-400 text-sm">Generating custom quiz...</p>
                </div>
              ) : questions.length === 0 ? (
                <div className="p-8 text-center text-slate-550 border border-dashed border-slate-800 rounded-2xl">
                  Failed to load quiz.
                </div>
              ) : (
            ...
            <div className="border border-dashed border-slate-800 rounded-2xl flex flex-col items-center justify-center text-center p-8 gap-4 w-full py-24">
              ...
              <h3 className="text-xl font-bold text-slate-400">AI Knowledge Quiz</h3>
              <p className="text-slate-550 max-w-sm text-sm">
                Select one of your documents from the sidebar to take a dynamically generated multiple-choice quiz.
              </p>
            </div>
          )}
        </div>
